              ...
              {pollData.options.map((option) => (
                <button
                  ...
                  className={`w-full p-3 rounded-lg text-lg font-medium flex justify-between items-center transition-all duration-300 
                    ${selectedOption === option._id ? "bg-blue-600" : "bg-gray-700 hover:bg-blue-500"}`}
                  onClick={()=>setSelectedOption(option._id)}
                >
                  <span>{option.text}</span>
                  <span className="bg-gray-900 text-gray-300 px-3 py-1 rounded-lg text-sm">{option.votes} votes</span>
                </button>
              ))}
            </div>

            <button className="w-2/5 p-3 rounded-lg text-lg font-medium flex justify-center items-center self-center transition-all duration-300 bg-pink-500 text-center mt-3" onClick={handleVote}>Submit</button>
          </div>
